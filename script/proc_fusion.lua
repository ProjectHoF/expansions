...
function Auxiliary.MultipleFusionTokensOperation(e,tp,eg,ep,ev,re,r,rp)
	for i=1,64 do
		...
		table.insert(GlobalMultipleFusionTokens[0],token)
	end
	for i=1,64 do
		local token=Duel.CreateToken(1,46448938)
		table.insert(GlobalMultipleFusionTokens[1],token)
	end
end
...
ge1:SetOperation(aux.MultipleFusionTokensOperation)
Duel.RegisterEffect(ge1,0)

function Auxiliary.MultipleFusionCheck(g)
	for tc in aux.Next(g) do
		local cc=GlobalMultipleFusionTable[tc]
		if cc and not g:IsContains(tc) then
			return false
		end
	end
	return true
...
function Fusion.ParseMaterialTable(tab,mat)
	local named_mats={}
	local tmp_extramat_func=function(c,fc,sub,sub2,mg,sg,tp,contact,sumtype)
		return (sub2 and c:IsHasEffect(511002961))
	end
	local name_func=function(_tab)
		return function(c,fc,sub,sub2,mg,sg,tp,contact,sumtype)
			if SatoneFusionFilter and SatoneFusionFilter(c,SatoneFusionEffect,SatoneFusionPlayer) then
				return true
			end
			return c:IsSummonCode(fc,sumtype,fc:GetControler(),table.unpack(_tab)) or (sub and c:CheckFusionSubstitute(fc)) or (sub2 and c:IsHasEffect(511002961))
		end
	end
	local function_func=function(func)
		return function(c,fc,sub,sub2,mg,sg,tp,contact,sumtype)
			if SatoneFusionFilter and SatoneFusionFilter(c,SatoneFusionEffect,SatoneFusionPlayer) then
				return true
			end
	...
	for i=1,#val do
		if type(val[i])=='function' then
			fun[i]=function(c,fc,sub,sub2,mg,sg,tp,contact,sumtype)
				...
				return (val[i](c,fc,sumtype,tp,sub,mg,sg,contact) or (sub2 and c:IsHasEffect(511002961))) and not c:IsHasEffect(6205579)
			end
...
function Fusion.CheckMixGoal(tp,sg,fc,sub,sub2,contact,sumtype,chkf,...)
	...
	return sg:IsExists(Fusion.CheckMix,1,nil,sg,g,fc,sub,sub2,contact,sumtype,tp,...) and
		...
		and aux.MultipleFusionCheck(sg)
end
